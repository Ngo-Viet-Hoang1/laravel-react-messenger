...
export default function UserOptionsDropdown({ channel }: Props) {
    ...
    const handleStartSecretChat = (): void => {
        if (channel.type !== 'direct' || !channel.peer_user_id || isLoading)
            return;

        setIsLoading(true);
        router.post(
            route('channels.secret-direct', channel.peer_user_id),
            {},
            {
                preserveScroll: true,
                onSuccess: () => {
    ...
    return (
        <Menu
            as="div"
            className="group absolute top-1/2 right-2 z-20 -translate-y-1/2"
        >
            <MenuButton
                disabled={isLoading}
                className="focus-visible:ring-phthalogreen-500 flex h-8 w-8 items-center justify-center rounded-full text-slate-500 opacity-0 transition-all group-hover:pointer-events-auto group-hover:bg-white/5 group-hover:opacity-100 group-hover:shadow-sm group-hover:shadow-slate-900/10 group-hover:backdrop-blur-sm hover:text-slate-700 focus-visible:ring-2 focus-visible:outline-none disabled:opacity-40 data-[open]:pointer-events-auto data-[open]:bg-white/5 data-[open]:opacity-100 data-[open]:shadow-sm data-[open]:shadow-slate-900/10 data-[open]:backdrop-blur-sm dark:text-slate-300 dark:group-hover:bg-slate-700/10 dark:group-hover:shadow-black/10 dark:hover:text-slate-100 dark:data-[open]:bg-slate-700/10 dark:data-[open]:shadow-black/10"
            >
                {isLoading ? (
