...
import { LockClosedIcon, LockOpenIcon, ShieldCheckIcon, UsersIcon } from "@heroicons/react/24/solid";
...
export default function UserOptionDropdown({ conversation }) {
    ...
    const changeUserRole = () => {
        ...
        axios.put(route("user.changeRole", conversation.id)
        ).then((res) => {
    ...
    const onBlockUser = () => {
        ...
        axios.put(route("user.blockUnblock", conversation.id)
        ).then((res) => {
            emit('toast.show', res.data.message);   
            console.log('res', res);
        }).catch((err) => {
